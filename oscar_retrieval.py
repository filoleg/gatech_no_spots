...
session = FuturesSession(max_workers=33)
...
def create_classList(sid, pin):
     ...
     subjects_data_list = ["201505","201508"]
     jk = 0
     for semester in subjects_data_list:
          #subjects_data = 'p_calling_proc=P_CrseSearch&p_term=201502'
          subjects_data = "p_calling_proc=P_CrseSearch&p_term=" + semester
          ...
          for i in raw_soup0:
               ...
               if c > 75:
                    break
          print(subjectList)
          secListName = "secList_" + semester + ".txt"
          secList = open(secListName, 'w')     
          for chosenSubject in subjectList:
               courses_data = 'rsts=dummy&crn=dummy&term_in='+ semester +'&sel_subj=dummy&sel_day=dummy&sel_schd=dummy&sel_insm=dummy&sel_camp=dummy&sel_levl=dummy&sel_sess=dummy&sel_instr=dummy&sel_ptrm=dummy&sel_attr=dummy&sel_subj=' + chosenSubject + '&sel_crse=&sel_title=&sel_from_cred=&sel_to_cred=&sel_ptrm=%25&begin_hh=0&begin_mi=0&end_hh=0&end_mi=0&begin_ap=x&end_ap=y&path=1&SUB_BTN=Course+Search'
...
def bg_cb(sess,resp):#background callback function for asynchronous http requests
     ...
     header_soup = soup.find_all("div", class_="staticheaders")
     semester = header_soup[0].get_text().splitlines()[2]
     ...
     while c < len(raw_soup):
          ...
          if section == u'\xa0' or raw_soup[1 + 20*(c//20)].get_text() == u'\xa0':#section or CRN
               ...
               if ((c+1)//20 != section_change):#once done with a section, add to the section collection and the db_id to sections of the class dictionary
                    ...
                    #sections.update({'CRN' : aSectionObj["CRN"]}, {"$set": {'students_registered' : aSectionObj['students_registered'], 'students_on_WL' : aSectionObj['students_on_WL']}}, True)
                    sections.update({'CRN' : aSectionObj["CRN"]}, aSectionObj, upsert=True)
                    changed = True     
               c+=1
          section_change = c//20
